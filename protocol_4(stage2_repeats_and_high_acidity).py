...
def run(protocol: protocol_api.ProtocolContext):
    ...
    tfa_dil_quantities = {'A1': 20, 'A2': 60, 'A3': 0, 'A4': 0, 'A5': 0, 'A6': 60, 'A7': 0, 'A8': 0, 'A9': 0, 'A10': 40, 'A11': 0, 'A12': 0} # Dilute TFA solution ()
    tfa_conc_quantities = {'A1': 0, 'A2': 0, 'A3': 24, 'A4': 36, 'A5': 60, 'A6': 0, 'A7': 24, 'A8': 36, 'A9': 60, 'A10': 0, 'A11': 36, 'A12': 60} # Concentrated TFA solution


    # DISPENSE FUNCTIONS    

    def premulti(solution, height_t0, well_count, total): # Proceedure to "prepare" each tip before use, ensuring accurate and consistent handling on solutions
    ...
    for plate, substrate in zip(plates, substrates): # Iterate through each plate and substrate - 1 columns of substrates (8 solutions) per 96-well plate
        ...
        for wells in plate.rows()[0]:
            liq_hand(100, substrate, wells, sub_depth_t0, substrate_count, sub_total)
    ...
    for plate in plates:
        for well, vol in hfip_quantities.items():
            if vol != 0:
                liq_hand(vol, hfip, plate[well], 0, [0], 1)
    ...
    for plate in plates:
        for well, vol in tfa_dil_quantities.items():
            if vol != 0:
                liq_hand(vol, tfa_dil, plate[well], 0, [0], 1)
    ...
    for plate in plates:
        for well, vol in tfa_conc_quantities.items():
            if vol != 0:
                liq_hand(vol, tfa_conc, plate[well], 0, [0], 1)
    ...
    # python.exe -m opentrons.simulate "C:\Users\crj21\OneDrive - Imperial College London\PhD\1st Year\CRJ-01-056\repeats_and_high_TFA.py" --custom-labware-path="C:\Users\crj21\AppData\Roaming\Opentrons\labware"